...
\pagestyle{fancy}
\fancyhf{}
\lhead{\small ERS/SRS -- SIMPA v3.1}
...
\vspace{0.15cm}
{\normalsize Revisión posterior al Comité de Control de Cambios:\par}
{\normalsize incorpora \textsc{rfc-01}, \textsc{rfc-02} y \textsc{rfc-03} (PE4 --- Unidad IV)\par}
\vspace{0.5cm}
\rule{\textwidth}{1.2pt}\par
\vspace{1.0cm}
...
{\bfseries Versión:} 3.1 \quad {\small(v1.1 en la numeración de la asignatura)} \qquad {\bfseries Fecha:} 4 de agosto de 2026\par
...
\rowcolor{grisclaro}3.1 & 04/08/2026 & Equipo AHMRV & \textbf{Revisión de control de cambios (PE4 --- Unidad IV).} Incorporación de las tres solicitudes de cambio aprobadas por el Comité de Control de Cambios (CCB): \id{RFC-01} ---nuevo \id{RF-40}, modo de operación degradada sin inferencia remota---, \id{RFC-02} ---modificación de \id{RNF-11} con política explícita de resolución de conflictos de sincronización concurrente--- y \id{RFC-03} ---nueva restricción \id{RD-11}, purga automática de los datos de geolocalización a los 12 meses. Actualización derivada de las §3.2, §3.3, §3.4, §3.5, §4.2, §4.3, §4.7, §5.1, §5.4 y §7.5. La trazabilidad de cada cambio se detalla en la §1.9. \\ \hline
...
\subsection{Incorporación de las solicitudes de cambio aprobadas por el CCB}
\label{sec:rfc}

La revisión 3.1 de este documento incorpora las tres solicitudes de cambio tramitadas durante la
práctica experimental de la Unidad IV y aprobadas por el Comité de Control de Cambios el
04/08/2026. El formulario completo de cada solicitud consta en el Anexo C del informe de la PE4 y
la deliberación correspondiente en el acta del CCB. Ninguna de las tres altera el alcance del
prototipo evaluado en la §6: \id{RFC-01} fue aprobada \emph{con condiciones}, con implementación
programada para la Entrega 4 (2B).

{\setlength{\tabcolsep}{4pt}
\begin{longtable}{|C{1.4cm}|L{2.0cm}|L{3.4cm}|L{5.2cm}|L{2.6cm}|}
\hline
\rowcolor{grisclaro}\textbf{RFC} & \textbf{Tipo} & \textbf{Requisitos afectados} & \textbf{Cambio incorporado} & \textbf{Decisión del CCB} \\
\hline
\endhead
\id{RFC-01} & Alcance & \id{RD-02}, \id{RD-05} (directos); \id{RNF-04}, \id{RNF-11}, \id{RNF-17}, \id{CU-01}, \id{CU-02}, \id{CU-06} (indirectos) & Alta del requisito funcional \id{RF-40}, ``modo de operación degradada sin inferencia remota'', que resuelve la incompatibilidad práctica entre la operación \emph{offline} (\id{RD-02}) y la inferencia centralizada en la nube (\id{RD-05}). & Aprobado con condiciones: se incorpora al ERS y a la matriz de trazabilidad; su implementación se programa para la Entrega 4, no para el MVP vigente. \\ \hline
\id{RFC-02} & Calidad & \id{RNF-11}, \id{RF-28}, \id{RF-35} (directos); \id{RF-29}, \id{RF-37} (indirectos) & Modificación de \id{RNF-11} para fijar una política explícita de resolución de conflictos de sincronización concurrente: \emph{last-write-wins} sobre \id{fechaRegistro}, con bitácora de conflicto accesible a la persona supervisora que efectuó el registro delegado. & Aprobado sin condiciones: la política es coherente con el patrón ya empleado en el flujo alternativo sin conectividad de \id{CU-04}. \\ \hline
\id{RFC-03} & Restricción normativa & \id{RL-06}, \id{RNF-14} (directos); \id{RF-14}, \id{RF-15}, \id{RF-16} (indirectos) & Alta de la restricción \id{RD-11}: purga automática de los datos individuales de geolocalización a los 12 meses de su captura, conservando únicamente los agregados no identificables empleados por \id{RF-16}. & Aprobado: \id{RL-06} ya obligaba a disponer de un mecanismo de supresión; \id{RD-11} lo hace explícito, automático y acotado en el tiempo. \\ \hline
\caption{Solicitudes de cambio incorporadas en la revisión 3.1}
\end{longtable}}

\noindent\textbf{Criterio de incorporación.} Cada cambio aprobado se refleja en tres lugares y no
solo en el enunciado del requisito: en la especificación de la §3, en los artefactos de modelado
afectados de la §4 y en la matriz de trazabilidad de la §5.4. Los cambios cuya realización queda
diferida a la Entrega 4 se declaran además en el trabajo pendiente de la §7.5, de modo que la
diferencia entre lo especificado y lo construido permanezca explícita.
...
Esta sección especifica 40 requisitos funcionales, 18 requisitos no funcionales cuantificados sobre
las nueve características de calidad de ISO/IEC 25010:2023 \cite{iso25010}, 11 restricciones de
diseño y el mapeo de requisitos legales a la LOPDP \cite{lopdp}. Los elementos incorporados o
modificados por las solicitudes de cambio aprobadas por el CCB ---\id{RF-40}, \id{RNF-11},
\id{RNF-14} y \id{RD-11}--- se señalan de forma expresa en el punto correspondiente (véase §1.9).
...
\RF{RF-40}{Modo de operación degradada sin inferencia remota}
{El sistema debe permitir que el cliente móvil continúe entregando la clasificación de madurez y el diagnóstico por imagen a partir de los últimos umbrales de variedad y de madurez sincronizados, cuando el servicio de inferencia remota resulte inalcanzable durante más de 4 horas continuas. Todo resultado obtenido en este modo debe presentarse marcado como ``no verificado en línea'' y reencolarse para su contraste cuando se restablezca la conexión.}
{Trabajador agrícola / Técnico fitosanitario · \id{EV-12}, \id{EV-07} · incorporado por \id{RFC-01}}
{\textbf{Entrada:} solicitud de diagnóstico y caché local de umbrales de la variedad del lote. \textbf{Salida:} resultado degradado con la marca ``no verificado en línea'' y la fecha de la última sincronización de umbrales.}
{\textbf{Pre:} existe caché local de umbrales y el servicio de IA lleva más de 4 horas continuas inalcanzable. \textbf{Post:} el resultado queda registrado como degradado y pendiente de contraste con la inferencia remota.}
{Should (implementación programada para la Entrega 4 por decisión del CCB)}
{Simulada la indisponibilidad del servicio de IA durante más de 4 horas continuas, el cliente entrega la clasificación empleando los umbrales en caché, la interfaz la presenta marcada como ``no verificado en línea'' y, restablecida la conexión, el resultado se contrasta con la inferencia remota y se actualiza si difiere.}

\noindent\textbf{Justificación del requisito.} Sin este mecanismo, \id{RD-02} ---operación sin
conexión--- y \id{RD-05} ---inferencia centralizada en la nube--- resultan mutuamente incompatibles
en la práctica: el 74,2\% del personal no dispone de conectividad permanente en su zona de trabajo
(\id{EV-12}), de modo que la ausencia de un modo degradado dejaría sin diagnóstico útil a la mayoría
de las jornadas de campo, y precisamente en la función mejor valorada del cuestionario (\id{RF-07},
4,27/5). El riesgo del cambio es que un diagnóstico calculado con umbrales desactualizados induzca
a error si la variedad del lote cambia entre sincronizaciones; se mitiga marcando todo resultado
degradado como no verificado y sometiéndolo a contraste posterior, nunca presentándolo con el mismo
estatus que un resultado verificado en línea.
...
\id{RNF-11} & Fiabilidad & La aplicación opera sin conexión y sincroniza el 100\% de los registros pendientes al restablecerse la conectividad, sin pérdida ni duplicación. Ante dos registros concurrentes del mismo avance ---por ejemplo, uno delegado (\id{RF-35}) y otro efectuado después por la propia persona trabajadora--- prevalece el de \id{fechaRegistro} más reciente (\emph{last-write-wins}); el registro descartado no se elimina, sino que se conserva en una bitácora de conflicto consultable por la persona supervisora que efectuó el registro delegado. & 20 operaciones ejecutadas sin conexión, verificadas tras la sincronización; y 5 pares de registros concurrentes sobre el mismo avance, verificando que prevalece el más reciente, que el avance no se computa dos veces y que el descarte consta en la bitácora de conflicto. & \id{EV-07}, \id{EV-10}, \id{RFC-02} \\ \hline
...
\id{RNF-14} & Seguridad & Los datos de geolocalización se cifran en reposo y su acceso queda registrado en bitácora con identificación de la persona usuaria, fecha y motivo. La ejecución de la purga exigida por \id{RD-11} deja constancia en la misma bitácora, con el número de registros suprimidos y el criterio de exención aplicado. & Auditoría de la bitácora tras 10 accesos de prueba y tras una ejecución de la tarea de purga sobre datos de antigüedad superior a 12 meses. & \footnotesize \cite{lopdp} Art. 37; \id{RFC-03} \\ \hline
...
\id{RL-06} & Art. 14 & Derecho de eliminación. & Supresión de los datos personales al término de la relación laboral, conservando los datos productivos de forma disociada. \id{RD-11} hace además automático y acotado en el tiempo el mecanismo de purga de los datos de geolocalización, con independencia de la vigencia del vínculo laboral. \\ \hline
...
\id{RD-11} & Los datos de geolocalización individual ---marcaciones GPS y recorridos--- se purgan automáticamente a los 12 meses de su captura; se conservan únicamente los agregados no identificables empleados por \id{RF-16} (evaluación de desempeño). Quedan excluidos de la purga los recorridos vinculados a una calificación desfavorable de la extractora mientras permanezca abierta la auditoría de calidad asociada (\id{RF-24}). & Principio de minimización de la LOPDP \cite{lopdp} (Art. 10, ya trazado en \id{RL-02}), que limita tanto el alcance como la duración del tratamiento. La especificación no fijaba plazo de retención para estos datos; \id{RD-11} cubre ese vacío y hace explícito el mecanismo de supresión que \id{RL-06} ya exige. Incorporada por \id{RFC-03}. \\ \hline
